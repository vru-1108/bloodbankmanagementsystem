...
var JotForm = {
     ...
     * @var String
     */
    server: "//www.jotform.com/server.html", // Will get the correct URL from this.getServerURL() method
    /**
    ...
    getServerURL: function () {
        ...
        if (form) {
            if (origin.include('.jotform.pro')) {
                this.server = origin + "/server.html";
                this.url = origin + '/';
                return;
            }

            if ((action = form.readAttribute('action'))) {
                if (action.include('submit.html') || action.include('server.html')) {
                    var n = !action.include('server.html') ? "submit" : "server";
                    this.server = action.replace(n + '.html', 'server.html');
                    this.url = action.replace(n + '.html', '');
                } else {
    ...
    */
    ...
    takeScreenShot: function (id) {
        ...
        //this.wishboxServer = '//ec2-107-22-70-25.compute-1.amazonaws.com/wishbox-bot.html';
        this.wishboxServer = 'https://screenshots.jotform.com/wishbox-server.html'; //kemal: made this http since https not working anyway
        //this.wishboxServer = "//beta23.jotform.com/server.html";//JotForm.server;
            ...
            function (res) {
                ...
                if (ie !== undefined && ie < 9) {
                    ...
                    b.click(function () {
                        ...
                        p.$jot.getJSON('https://screenshots.jotform.com/wishbox-server.html?callback=?', {
                            action: 'getImage',
                            uniqID: $this.scuniq
                        }, function (res) {
                            if (!res.success) {
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    uniqid: function (prefix, more_entropy) {
        ...
        if (!this.html_js.uniqidSeed) {
            this.html_js.uniqidSeed = Math.floor(Math.random() * 0x75bcd15);
        }
        this.html_js.uniqidSeed++;
        retId = prefix;
        retId += formatSeed(parseInt(new Date().getTime() / 1000, 10), 8);
        retId += formatSeed(this.html_js.uniqidSeed, 5);
        if (more_entropy) {
    ...
    handleSavedForm: function () {
        ...
        var a = new Ajax.Jsonp(JotForm.url + '/server.html', {
            parameters: parameters,
            evalJSON: 'force',
            onComplete: function (t) {
                var res = t.responseJSON;
        ...
        setTimeout(function () {
            ...
            if (IE && IE < 9) {
                ...
                if (UA.indexOf('windows nt 5.1') != -1 || UA.indexOf('windows xp') != -1 || UA.indexOf('windows nt 5.2') != -1) {
                    // windows XP
                    JotForm.server = "https://www.jotform.com/server.html";
                }
            }
  ...
  */
  ...
  /**
    ...
    getClientCountry: function(callback) {
      new Ajax.Request('//china.jotfor.ms/opt/geo.ip.html', {
        evalJSON: 'force',
    ...
    initOwnerView: function(formID) {
        ...
        var src = url + '/ownerView.html?id=' + formID;
